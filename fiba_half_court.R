...
ggplot(data=data.frame(x=1,y=1),aes(x,y))+
  ###floor:
  geom_rect(aes(xmin = -0.5, xmax = 15.5, ymin = -0.5, ymax = 14.5),fill = "darkgoldenrod", alpha = 0.7) +
  geom_rect(aes(xmin = 5.05, xmax = 9.95, ymin = 8.2, ymax = 14),fill = "black", alpha = 0.85) +
  ###court:
  geom_path(data=data.frame(x=c(0,0,15,15,0),y=c(0,14,14,0,0)), colour = "white")+
  ###centre circle:
